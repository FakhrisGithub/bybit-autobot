...
def ensure_one_way_mode(session, symbol: str):
    """Pastikan position mode = One-Way (positionIdx=0) supaya tidak kena error 10024."""
    try:
        session.switch_position_mode(category="linear", symbol=symbol, mode=0)
    except Exception as e:
        err = str(e).lower()
        # "position mode is not modified" = sudah one-way, aman diabaikan
        if "not modified" not in err and "110025" not in err:
            print(f"[executor] switch_position_mode warning {symbol}: {e}")
...
def open_position(session, signal: dict) -> dict | None:
    ...
    ensure_one_way_mode(session, symbol)
    ...
    try:
        ...
        err_str = str(e)
        if "10024" in err_str:
            if symbol not in config.EXCLUDE_SYMBOLS:
                config.EXCLUDE_SYMBOLS.append(symbol)
                print(f"[executor] {symbol} di-blacklist (10024 regulatory). "
                      f"Total blacklist: {len(config.EXCLUDE_SYMBOLS)}")
        else:
