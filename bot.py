...
def get_all_usdt_symbols(session) -> list:
    ...
    for x in info["result"]["list"]:
        ...
        if sym in config.EXCLUDE_SYMBOLS:
            continue
        symbols.append(sym)
    print(f"[bot] Total instrumen USDT-linear ditemukan: {len(symbols)}")
    return symbols
...
def filter_by_turnover(session, symbols: list) -> list:
    ...
    try:
        ...
        filtered = [s for s in symbols if turnover_map.get(s, 0) >= config.MIN_TURNOVER_24H_USDT]
        print(f"[bot] Setelah filter turnover (>={config.MIN_TURNOVER_24H_USDT:,.0f} USDT): "
              f"{len(filtered)} dari {len(symbols)} simbol")
        return filtered
...
def main():
    ...
    if config.LIVE and config.LIVE_CONFIRM != "YES_I_UNDERSTAND_THE_RISK":
        raise SystemExit(
            "BYBIT_MODE=live tapi LIVE_CONFIRM belum di-set dengan benar.\n"
            "Ini uang sungguhan. Untuk lanjut, set environment variable:\n"
            "  LIVE_CONFIRM=YES_I_UNDERSTAND_THE_RISK\n"
            "Pastikan kamu sudah cek ulang semua angka di config.py / .env "
            "(MARGIN_PER_TRADE_USDT, LEVERAGE, MAX_CONCURRENT_POSITIONS, "
            "DAILY_LOSS_LIMIT_PCT) sebelum melakukan ini."
        )
    ...
    print(f"[bot] Starting. MODE={config.BYBIT_MODE} (testnet={config.TESTNET}, demo={config.DEMO}, live={config.LIVE}) | "
          ...
          f"MAX_CONSECUTIVE_LOSSES={config.MAX_CONSECUTIVE_LOSSES}")

    if config.LIVE:
        print("[bot] ⚠️  LIVE MODE AKTIF — INI UANG SUNGGUHAN. Bot akan entry/exit otomatis tanpa konfirmasi.")
