...
def main():
    ...
    # Raw HTTP test, bypass pybit sepenuhnya, untuk lihat response mentah Bybit
    ...
    try:
        ...
        _url = f"https://api-testnet.bybit.com/v5/account/wallet-balance?{_query}"
        ...
        with _ur.urlopen(_req, timeout=10) as r:
            print(f"[debug] RAW HTTP test response: {_json.loads(r.read().decode())}")
    except _ue.HTTPError as e:
        print(f"[debug] RAW HTTP test HTTPError {e.code}: {e.read().decode()}")
    except Exception as e:
        print(f"[debug] RAW HTTP test exception: {e}")

    # Cek server time Bybit vs server time Railway (deteksi clock skew)
    try:
        _server_time_resp = _ur.urlopen("https://api-testnet.bybit.com/v5/market/time", timeout=10)
        _server_time = _json.loads(_server_time_resp.read().decode())
        _bybit_ts = int(_server_time["result"]["timeSecond"]) * 1000
        _local_ts = int(_time.time() * 1000)
        print(f"[debug] Bybit server time={_bybit_ts} | Railway local time={_local_ts} | "
              f"diff_ms={_local_ts - _bybit_ts}")
    except Exception as e:
        print(f"[debug] server time check failed: {e}")
    # ------------------------------------------------------------------------------------------
